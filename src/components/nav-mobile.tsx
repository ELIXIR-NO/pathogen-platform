      ...
      {menuOpen && (
        <div className="absolute top-full left-0 right-0 bg-white shadow-md">
          <ul className="flex flex-col items-start justify-start gap-y-4 p-4">
            <NavMenuItem title="Topics" items={topics} onClose={() => setMenuOpen(false)} />
            <NavMenuItem title="Pandemic Preparedness" items={pandemicPreparednessNavItems} onClose={() => setMenuOpen(false)} />
            <NavMenuItem title="RDM" items={RDMNavDropdownItems} onClose={() => setMenuOpen(false)} />
            <NavMenuItem title="One Health" items={oneHealthNavItems} onClose={() => setMenuOpen(false)} />
            {NavItems.map((it) => (
              <NavBarItem
                key={it.pageName}
                pageUrl={it.pageUrl}
...
const NavMenuItem: React.FC<{ title: string; items: DropdownMenuItem[]; onClose: () => void }> = ({ title, items, onClose }) => {
...
const NavBarItem: React.FC<NavItem & { onClick: () => void }> = ({ pageUrl, pageName, onClick }) => (
  <li>
    <Link
